...
export class MechanicsController {
  ...
  async getWorkTimeById(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
  ...
  async updateWorkTime(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      ...
      // Calculate total work time
      const finalWorkTime =
        workTime !== undefined ? workTime : existingWorkTime.workTime;
      const finalPauseTime =
        pauseTime !== undefined ? pauseTime : existingWorkTime.pauseTime;
      ...
      if (date !== undefined) updateData.date = workDate;
      if (workTime !== undefined) updateData.workTime = workTime;
      if (pauseTime !== undefined) updateData.pauseTime = pauseTime;
  ...
  async pauseActivity(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      ...
      const updated = await this.prisma.activityMechanic.update({
        ...
        data: {
          status: "DELAYED",
          pausedAt: new Date(),
          pauseReason: pauseReason ? (pauseReason as any) : null,
        },
        include: {
          mechanic: {
            select: {
  ...
  async resumeActivity(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
