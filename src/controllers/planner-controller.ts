...
export class PlannerController {
  ...
  async getGroupLeaders(req: Request, res: Response): Promise<void> {
    try {
  ...
  async getMechanicsReport(req: Request, res: Response): Promise<void> {
    try {
      ...
      const calculateMechanicStats = (mechanicList: any[], assignmentsList: any[]) => {
        return mechanicList.map((mechanic) => {
          const assignments = assignmentsList.filter(
            (aa) => aa.mechanicId === mechanic.id
          );

          const activities = assignments.map((assignment) => {
            // Calculate time for each task
            const tasksWithTime = assignment.tasks.map((task: any) => {
              let durationSeconds = 0;
              let isActive = false;
            ...
            const totalActivitySeconds = tasksWithTime.reduce(
              (sum: number, task: any) => sum + task.durationSeconds,
              0
            );
      ...
      const allMechanicsStats = calculateMechanicStats(
        allMechanicsForStats,
        allFilteredAssignmentsForStats
      );
      const aggregateStats = {
        totalMechanics: allMechanicsStats.length,
        totalActivities: allMechanicsStats.reduce(
          (sum, m) => sum + m.totalActivities,
          0
        ),
        totalWorkTime: allMechanicsStats.reduce(
          (sum, m) => sum + m.totalMechanicSeconds,
          0
        ),
      };
  ...
  async getMechanicReportById(req: Request, res: Response): Promise<void> {
    try {
      ...
      const calculateMechanicStats = (mechanicData: any, assignmentsList: any[]) => {
        const assignments = assignmentsList.filter(
          (aa) => aa.mechanicId === mechanicData.id
        );
  ...
  private async getMechanicsReportData(search?: string) {
    const mechanicWhere: any = {
      posisi: "MEKANIK",
    };
  ...
  async downloadMechanicsReportPDF(req: Request, res: Response): Promise<void> {
    try {
      const { search } = req.query;
      const searchValue = search ? (Array.isArray(search) ? search[0] : search) : undefined;

      const report = await this.getMechanicsReportData(searchValue as string);

      const doc = new PDFDocument({ margin: 50 });
      res.setHeader("Content-Type", "application/pdf");
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=mechanics-report-${new Date().toISOString().split("T")[0]}.pdf`
      );
        ...
        if (mechanic.activities.length > 0) {
          ...
          mechanic.activities.forEach((activity) => {
            doc.fontSize(10).text(
              `• ${activity.activityName.replace(/_/g, " ")} - ${activity.unitCode} (${activity.unitType} ${activity.unitBrand})`,
              { indent: 20 }
            );
  ...
  async downloadMechanicsReportExcel(req: Request, res: Response): Promise<void> {
    try {
      const { search } = req.query;
      const searchValue = search ? (Array.isArray(search) ? search[0] : search) : undefined;
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=mechanics-report-${new Date().toISOString().split("T")[0]}.xlsx`
      );
  ...
  async downloadMechanicReportExcelById(req: Request, res: Response): Promise<void> {
    try {
      const { mechanicId } = req.params;
      const id = Array.isArray(mechanicId) ? mechanicId[0] : mechanicId;
  ...
  async getActivityAnalytics(req: Request, res: Response): Promise<void> {
    try {
      ...
      const timeByActivityType: Record<string, number> = {};
  ...
  async assignGroupLeader(req: Request, res: Response): Promise<void> {
    try {
      const userId = req.user?.id;
      ...
      if (
        groupLeader.posisi !== "GROUP_LEADER_MEKANIK" &&
